    ...
    try {
        const response = await axios.post(
            `https://api.on-demand.io/chat/v1/sessions/${sessionId}/query`,
            {
                endpointId: 'predefined-openai-gpt4o',
                query: userQuery,
                pluginIds: ['plugin-1726258392', 'plugin-1726268880'],
                responseMode: 'sync'
            },
            {
                headers: {
...
module.exports.learningPath = async (req, res) => {
    try {
        ...
        const learningPathResponse = await main(`Give me the learning path for ${skill} with the this previous knowleadge ${skillLevel} make good roadmap  `, 'learningPath');
